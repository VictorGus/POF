...
(pages/reg-subs-page
 model/index
 (let [login-on-enter #(-> (.getElementById js/document "password-form")
                           (.addEventListener "keyup"
                                              (fn [e]
                                                (if (= (aget e "keyCode") 13)
                                                  (do (.preventDefault e) (.click (.getElementById js/document "login-btn")))))))]
   (r/create-class
    {:component-did-mount
     (fn [_]
       (login-on-enter))
           ...
           [:div.card-body
            ...
            [:div.card-body.border
             ...
             [basic-form/form-password [form/form-path :password]
              "Enter password"]
             [:button#login-btn.btn.btn-outline-primary.btn-block.mt-3.mb-2
